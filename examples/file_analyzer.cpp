...
public:
    ...
    void analyzeDirectory(const std::string& dirpath) {
        ...
        if (!large_files.empty()) {
            ...
            for (const auto& file : large_files) {
                std::cout << "  " << file.first << " (" << formatFileSize(file.second) << ")"
                          << std::endl;
            }
        }
    ...
    void generateReport(const std::string& path, const std::string& output_file = "") {
        ...
        std::string filename =
            output_file.empty() ? "file_analysis_report_" + getTimestamp() + ".txt" : output_file;
...
private:
    ...
    std::vector<std::pair<std::string, size_t>> getLargestFiles(const std::string& dirpath,
                                                                int count) {
    ...
    std::string formatPermissions(std::filesystem::perms perms) {
        std::string result = "";
        result += (perms & std::filesystem::perms::owner_read) != std::filesystem::perms::none
                      ? "r"
                      : "-";
        result += (perms & std::filesystem::perms::owner_write) != std::filesystem::perms::none
                      ? "w"
                      : "-";
        result += (perms & std::filesystem::perms::owner_exec) != std::filesystem::perms::none
                      ? "x"
                      : "-";
        result += (perms & std::filesystem::perms::group_read) != std::filesystem::perms::none
                      ? "r"
                      : "-";
        result += (perms & std::filesystem::perms::group_write) != std::filesystem::perms::none
                      ? "w"
                      : "-";
        result += (perms & std::filesystem::perms::group_exec) != std::filesystem::perms::none
                      ? "x"
                      : "-";
        result += (perms & std::filesystem::perms::others_read) != std::filesystem::perms::none
                      ? "r"
                      : "-";
        result += (perms & std::filesystem::perms::others_write) != std::filesystem::perms::none
                      ? "w"
                      : "-";
        result += (perms & std::filesystem::perms::others_exec) != std::filesystem::perms::none
                      ? "x"
                      : "-";
        return result;
    }
...
void printUsage() {
    ...
    std::cout << "  ./file_analyzer ollama <model> [mode] -report <path> [output_file]"
              << std::endl;
