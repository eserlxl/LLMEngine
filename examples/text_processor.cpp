...
class TextProcessor {
  ...
  public:
    ...
    void summarizeText(const std::string& text, const std::string& output_file = "") {
        ...
        try {
            std::string prompt =
                "Please provide a comprehensive summary of the following text. Include the main "
                "points, key insights, and important details. Make it clear and well-structured.";
    ...
    void processBatch(const std::string& input_dir, const std::string& operation) {
        ...
        std::cout << "\n📁 Processing " << text_files.size() << " files in " << input_dir
                  << std::endl;
        ...
        for (const auto& file : text_files) {
            ...
            if (!content.empty()) {
                std::string output_file =
                    std::filesystem::path(file).stem().string() + "_" + operation + ".txt";
...
void printUsage() {
    ...
    std::cout
        << "  ./text_processor ollama <model> [mode] -t <text> <target_language> [source_language]"
        << std::endl;
    ...
    std::cout << "  ./text_processor ollama qwen3:4b chat -s \"Long text here...\" summary.txt"
              << std::endl;
    std::cout << "  ./text_processor ollama qwen3:4b generate -k \"Text to analyze...\" 15"
              << std::endl;
...
int main(int argc, char* argv[]) {
    ...
    try {
        ...
        if (operation == "-s" || operation == "--summarize") {
            ...
            if (argc < 4) {
                std::cerr << "❌ Batch operation requires directory and operation type!"
                          << std::endl;
                return 1;
            }
