...
APIConfigManager::APIConfigManager()
    : defaultConfigPath_(std::string(::LLMEngine::Constants::FilePaths::DEFAULT_CONFIG_PATH)) {}
...
bool validateConfig(const nlohmann::json& config, ::LLMEngine::Logger* logger) {
    ...
    if (config.contains(std::string(::LLMEngine::Constants::JsonKeys::PROVIDERS))) {
        ...
        for (const auto& [providerName, providerConfig] : providers.items()) {
            ...
            if (providerConfig.contains(std::string(::LLMEngine::Constants::JsonKeys::BASE_URL))) {
                const auto& baseUrl =
                    providerConfig[std::string(::LLMEngine::Constants::JsonKeys::BASE_URL)];
            ...
            if (providerConfig.contains(
                ...
                const auto& model =
                    providerConfig[std::string(::LLMEngine::Constants::JsonKeys::DEFAULT_MODEL)];
                if (model.is_string()) {
    ...
    if (config.contains(std::string(::LLMEngine::Constants::JsonKeys::TIMEOUT_SECONDS))) {
        ...
        if (timeout.is_number_integer()) {
            ...
            if (timeoutVal < MIN_TIMEOUT_SECONDS || timeoutVal > MAX_TIMEOUT_SECONDS) {
                if (logger) {
                    logger->log(
...
int APIConfigManager::getTimeoutSeconds(std::string_view providerName) const {
    std::shared_lock<std::shared_mutex> lock(mutex_);
    int defaultTimeout = ::LLMEngine::Constants::DefaultValues::TIMEOUT_SECONDS;
