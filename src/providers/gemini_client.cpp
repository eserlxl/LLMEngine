...
GeminiClient::GeminiClient(const std::string& apiKey, const std::string& model)
    : apiKey_(apiKey), model_(model),
      baseUrl_(std::string(::LLMEngine::Constants::DefaultUrls::geminiBase)) {
...
nlohmann::json GeminiClient::buildPayload(std::string_view prompt,
    ...
    if (input.contains(std::string(::LLMEngine::Constants::JsonKeys::systemPrompt))
        && input[std::string(::LLMEngine::Constants::JsonKeys::systemPrompt)].is_string()) {
        userText += input[std::string(::LLMEngine::Constants::JsonKeys::systemPrompt)]
                         .get<std::string>();
...
APIResponse GeminiClient::sendRequest(std::string_view prompt,
    ...
    try {
        ...
        timeout_seconds =
            std::min(timeout_seconds, ::LLMEngine::Constants::DefaultValues::maxTimeoutSeconds);
