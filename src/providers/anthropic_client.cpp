...
AnthropicClient::AnthropicClient(const std::string& apiKey, const std::string& model)
    : apiKey_(apiKey), model_(model),
      baseUrl_(std::string(::LLMEngine::Constants::DefaultUrls::anthropicBase)) {
    defaultParams_ = {{"max_tokens", ::LLMEngine::Constants::DefaultValues::maxTokens},
...
nlohmann::json AnthropicClient::buildPayload(std::string_view prompt,
    ...
    if (input.contains(std::string(::LLMEngine::Constants::JsonKeys::systemPrompt))) {
        payload["system"] = input[std::string(::LLMEngine::Constants::JsonKeys::systemPrompt)]
                                .get<std::string>();
    }
    return payload;
}
...
APIResponse AnthropicClient::sendRequest(std::string_view prompt,
    ...
    try {
        ...
        if (params.contains(std::string(::LLMEngine::Constants::JsonKeys::timeoutSeconds))) {
            timeoutSeconds =
                params[std::string(::LLMEngine::Constants::JsonKeys::timeoutSeconds)].get<int>();
        } else {
            timeoutSeconds = config_ ? config_->getTimeoutSeconds()
                                      : APIConfigManager::getInstance().getTimeoutSeconds();
        }
        // Clamp to safe range [1, maxTimeoutSeconds] seconds
        timeoutSeconds = std::max(timeoutSeconds, 1);
        timeoutSeconds =
            std::min(timeoutSeconds, ::LLMEngine::Constants::DefaultValues::maxTimeoutSeconds);
        ...
        cpr::Response cprResponse = sendWithRetries(
            ...
            [&]() {
                return cpr::Post(cpr::Url{url},
                                 ...
                                 cpr::Timeout{timeoutSeconds * millisecondsPerSecond},
