...
OllamaClient::OllamaClient(const std::string& baseUrl, const std::string& model)
    ...
    defaultParams_ = {{"temperature", ::LLMEngine::Constants::DefaultValues::temperature},
                       ...
                       {"top_k", ::LLMEngine::Constants::DefaultValues::topK},
                       {"min_p", ::LLMEngine::Constants::DefaultValues::minP},
                       {"context_window", ::LLMEngine::Constants::DefaultValues::contextWindow}};
}
...
APIResponse OllamaClient::sendRequest(std::string_view prompt,
    ...
    try {
        ...
        auto get_timeout_seconds = [&](const nlohmann::json& p) -> int {
            if (p.contains(std::string(::LLMEngine::Constants::JsonKeys::timeoutSeconds))) {
                return p.at(std::string(::LLMEngine::Constants::JsonKeys::timeoutSeconds))
                    .get<int>();
            }
