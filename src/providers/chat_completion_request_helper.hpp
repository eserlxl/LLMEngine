...
struct ChatMessageBuilder {
    static nlohmann::json buildMessages(std::string_view prompt, const nlohmann::json& input) {
        ...
        if (input.contains("messages") && input["messages"].is_array()) {
            ...
            if (input.contains(std::string(::LLMEngine::Constants::JsonKeys::systemPrompt))) {
                const auto& sp =
                    input.at(std::string(::LLMEngine::Constants::JsonKeys::systemPrompt));
                if (sp.is_string()) {
...
struct ChatCompletionRequestHelper {
    ...
    static APIResponse execute(const nlohmann::json& default_params,
        ...
        try {
            ...
            if (options.generation.user.has_value() || 
                ...
                if (options.generation.logprobs.has_value()) {
                    ...
                    if (options.generation.top_logprobs.has_value()) {
                        final_params_storage[std::string(::LLMEngine::Constants::JsonKeys::topLogprobs)] = *options.generation.top_logprobs;
                    }
                }
                if (options.generation.top_k.has_value()) {
                    final_params_storage[std::string(::LLMEngine::Constants::JsonKeys::topK)] = *options.generation.top_k;
                }
                if (options.generation.min_p.has_value()) {
                    final_params_storage[std::string(::LLMEngine::Constants::JsonKeys::minP)] = *options.generation.min_p;
                }
            ...
            if (options.timeout_ms.has_value()) {
                ...
                timeout_seconds = (*options.timeout_ms + (::LLMEngine::Constants::DefaultValues::millisecondsPerSecond - 1)) / 
                                  ::LLMEngine::Constants::DefaultValues::millisecondsPerSecond;
            } else if (params.contains(
                           std::string(::LLMEngine::Constants::JsonKeys::timeoutSeconds))
                       && params.at(std::string(::LLMEngine::Constants::JsonKeys::timeoutSeconds))
                              .is_number_integer()) {
                timeout_seconds =
                    params.at(std::string(::LLMEngine::Constants::JsonKeys::timeoutSeconds))
                        .get<int>();
            ...
            // Clamp to a safe range [1, maxTimeoutSeconds] seconds
            timeout_seconds = std::max(timeout_seconds, 1);
            timeout_seconds = std::min(timeout_seconds,
                                       ::LLMEngine::Constants::DefaultValues::maxTimeoutSeconds);
            ...
            if (connect_timeout_ms > ::LLMEngine::Constants::DefaultValues::maxConnectTimeoutMs)
                connect_timeout_ms = ::LLMEngine::Constants::DefaultValues::maxConnectTimeoutMs;
                ...
                [&]() {
                    if (connect_timeout_ms > 0) {
                        return cpr::Post(cpr::Url{url},
                                         ...
                                         cpr::Timeout{timeout_seconds * millisecondsPerSecond},
    ...
    static void executeStream(const nlohmann::json& default_params,
        ...
        cpr::Response response = cpr::Post(cpr::Url{url},
                                           cpr_headers,
                                           cpr::Body{serialized_body},
                                           cpr::Timeout{timeout_seconds * ::LLMEngine::Constants::DefaultValues::millisecondsPerSecond},
                                           cpr::VerifySsl{verify_ssl},
