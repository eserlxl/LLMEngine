...
constexpr size_t commandBufferCount = 256;
constexpr size_t maxOutputLines = 10000;
constexpr size_t maxLineLength = static_cast<size_t>(1024) * 1024; // 1MB
constexpr size_t maxCmdStringLength = 4096;
constexpr size_t maxArgCount = 64;
constexpr size_t maxArgLength = 512;
...
std::vector<std::string> execCommandImpl(const std::vector<std::string>& args,
    ...
    if (cmdStrForLogging.size() > maxCmdStringLength) {
        if (logger) {
            logger->log(::LLMEngine::LogLevel::Error,
    ...
    for (const auto& a : args) {
        if (a.size() > maxArgLength || !std::regex_match(a, safeCharsRegex)) {
            if (logger) {
                logger->log(::LLMEngine::LogLevel::Error,
    ...
    if (!std::regex_match(cmdStrForLogging, safeCharsRegex)) {
        if (logger) {
            logger->log(::LLMEngine::LogLevel::Error,
    ...
    std::array<char, commandBufferCount> buffer;
    ...
    while (!stdoutClosed || !stderrClosed) {
        ...
        if (!stdoutClosed && (pollfds[0].revents & POLLIN)) {
            ...
            if (bytesRead > 0) {
                ...
                if (stdoutTotalSize > maxLineLength * maxOutputLines) {
                    if (logger) {
                        logger->log(::LLMEngine::LogLevel::Warn,
        ...
        if (!stderrClosed && (pollfds[1].revents & POLLIN)) {
            ...
            if (bytesRead > 0) {
                ...
                if (stderrTotalSize > maxLineLength * maxOutputLines) {
                    if (logger) {
                        logger->log(::LLMEngine::LogLevel::Warn,
    ...
    std::string line;
    while (std::getline(stdoutStream, line) && totalLines < maxOutputLines) {
    ...
    while (std::getline(stderrStream, line) && totalLines < maxOutputLines) {
        if (line.size() > maxLineLength) {
            line.resize(maxLineLength);
        }
        output.push_back(line);
...
std::vector<std::string> execCommand(std::string_view cmd, ::LLMEngine::Logger* logger) {
    ...
    if (cmdStr.size() > maxCmdStringLength) {
