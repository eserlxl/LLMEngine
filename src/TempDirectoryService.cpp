...
TempDirectoryService::DirectoryResult TempDirectoryService::ensureSecureDirectory(
    ...
    if (std::filesystem::exists(directory_path, ec)) {
        ...
        if (std::filesystem::is_symlink(directory_path, ec_symlink)) {
            result.error_message = "Temporary directory cannot be a symlink for security reasons: " + directory_path;
            if (logger) {
    ...
    if (ec) {
        result.error_message = "Failed to create temporary directory: " + directory_path + ": " + ec.message();
        if (logger) {
            logger->log(LogLevel::Error, result.error_message);
        }
        return result;
    ...
    if (std::filesystem::exists(directory_path)) {
        ...
        if (ec_perm && logger) {
            logger->log(LogLevel::Warn, std::string("Failed to set permissions on ") + directory_path + ": " + ec_perm.message());
        }
    }
...
bool TempDirectoryService::validatePathWithinRoot(
    ...
    try {
        ...
        const std::filesystem::path requested = std::filesystem::path(requested_path).lexically_normal();
        
        // Ensure requested is under default_root using a robust relative check
        const std::filesystem::path rel = std::filesystem::weakly_canonical(requested).lexically_relative(
        ...
        if (!is_within && logger) {
            logger->log(LogLevel::Warn, std::string("Rejected temp directory outside allowed root: ") + requested.string());
        }
        
        return is_within;
    } catch (...) {
        if (logger) {
            logger->log(LogLevel::Error, std::string("Failed to validate temp directory due to path error: ") + requested_path);
        }
        return false;
    }
}
...
bool TempDirectoryService::isDirectoryValid(const std::string& directory_path, Logger* logger) const {
    ...
    if (std::filesystem::is_symlink(directory_path, ec_symlink)) {
        if (logger) {
            logger->log(LogLevel::Error, std::string("Temporary directory is a symlink: ") + directory_path);
        }
        return false;
    }
