...
void testUrlRedactionEncodedDelimiters() {
    ...
    std::string result2 = RequestLogger::redactUrl(url2);
    // The implementation should decode %3D to = for matching, so "api=key" should match "api_key" pattern
    // This tests the basic URL decoding in parameter names
    ...
    assert(result4.find("api_key=<REDACTED>") != std::string::npos && "Should still redact sensitive params");
    
    // Test 5: Semicolon-separated parameters (RFC 1866)
    std::string url5 = "https://api.example.com/endpoint?param1=value1;api_key=secret;param2=value2";
    std::string result5 = RequestLogger::redactUrl(url5);
    assert(result5.find("api_key=<REDACTED>") != std::string::npos && "Should handle semicolon separators");
    assert(result5.find("param1=value1") != std::string::npos && "Should preserve non-sensitive params");
...
void testHeaderRedaction() {
    ...
    assert(result1.find("Authorization") == result1.end() && "Non-allowed sensitive header should be omitted");
    ...
    assert(result6["Content-Type"] == "application/json; charset=utf-8" && "Should preserve special chars in values");
    ...
    std::map<std::string, std::string> headers7 = {
        {"Content-Type", "application/json"},
        {"Content-Length", "1024"},
        {"Accept", "application/json"}
    };
...
void testTextRedaction() {
    ...
    assert(result3b.find("<REDACTED>") != std::string::npos && "Single-quoted value should be redacted");
    ...
    assert(result8.find("<REDACTED>") != std::string::npos && "Value with special chars should be redacted");
    ...
    assert(result10.find("<REDACTED>") != std::string::npos && "Value with delimiter should be redacted");
...
void testUrlRedactionFragmentHandling() {
    ...
    assert(result1.find("api_key=<REDACTED>") != std::string::npos && "Should redact before fragment");
...
void testUrlRedactionSemicolonSeparators() {
    ...
    std::string url1 = "https://api.example.com/endpoint?param1=value1;api_key=secret;param2=value2";
    std::string result1 = RequestLogger::redactUrl(url1);
    assert(result1.find("api_key=<REDACTED>") != std::string::npos && "Should handle semicolon separators");
    assert(result1.find("param1=value1") != std::string::npos && "Should preserve non-sensitive params");
    
    // Test 2: Mixed & and ; separators
    std::string url2 = "https://api.example.com/endpoint?param1=value1&api_key=secret;param2=value2";
    std::string result2 = RequestLogger::redactUrl(url2);
    assert(result2.find("api_key=<REDACTED>") != std::string::npos && "Should handle mixed separators");
...
void testPerformanceLargePayloads() {
    ...
    assert(result_headers.find("Content-Type") != result_headers.end() && "Should process large header map");
