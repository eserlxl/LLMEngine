...
int main() {
    ...
    // Test that createClientFromConfig throws when no credentials are provided for non-Ollama providers
    // This is the new security behavior - fail fast instead of proceeding silently
    {
        ...
        assert(exception_thrown && "createClientFromConfig should throw when no credentials provided");
    }

    // Test that Ollama still works without credentials (it's local)
    {
        nlohmann::json ollama_config = {{"base_url", "http://localhost:11434"}, {"default_model", "llama2"}};
