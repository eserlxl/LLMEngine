...
struct EnvBackup {
    ...
    EnvBackup() {
        ...
        if (qwen) qwen_key = qwen;
        if (openai) openai_key = openai;
        if (anthropic) anthropic_key = anthropic;
    }
...
void testCredentialResolution() {
    ...
    try {
        ...
        if (client) {
            ...
            std::cout << "  (Environment variable should be preferred over config file)" << std::endl;
        } else {
        ...
        if (client2) {
            ...
            std::cout << "  (Should log warning about using config file for credentials)" << std::endl;
        } else {
...
public:
    ...
    APIResponse sendRequest(std::string_view, const nlohmann::json&, const nlohmann::json&) const override {
    ...
    std::string getProviderName() const override { return "FailingTest"; }
...
void testAnalyzeErrorPaths() {
    ...
    try {
        ...
        LLMEngine engine(std::move(error_client), {}, 1, false);
        ...
        LLMEngine engine2(std::move(timeout_client), {}, 1, false);
        ...
        LLMEngine engine3(std::move(auth_error_client), {}, 1, false);
